...
def rag_answer(query):
    ...
    # Gate 1 — 파이프라인 최앞단의 결정론적 룰 필터(LLM 없음). @observe 로 열린 이 trace 아래에
    # gate1_rulebase span 을 첫 자식으로 남기고, EXIT 면 검색·LLM 을 아예 타지 않고 고정 응답을
    # 그대로 돌려준다(순수 삽입 — CONTINUE 면 아래 기존 흐름이 그대로 이어진다).
