...
잡아도 내용 오표기가 남았다 — 라벨 데이터 106건 실측에서 마커는 근거를 실제로 쓴 답변의
33/61(54%)에서 출처를 잘못 생략했다(study/2026-07-30.md). 프롬프트 수정 7종이 전부
실패해, 업계 표준(AWS Bedrock 가드레일, Google Vertex Check Grounding)과 같은 "생성 후
결정론적 판정" 구조로 바꿨다. 같은 라벨에서 이 판정기는 누락 1/61, 거절문 차단 24/45.
