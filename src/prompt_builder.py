...
# Gate3(검색 관련도 게이트, candidate_ranking.gate3_exit) 전용 고정 문구 — OUT_OF_SCOPE_MESSAGE와
# 의미가 다르므로 재사용하지 않는다. Gate2는 "이 주제 자체가 KDIC 업무가 아님"이 확정된 경우고,
# Gate3는 "주제가 업무 범위일 수도 있지만 이 표현으로는 코퍼스에서 직접 관련된 자료를 못 찾음"인
# 경우다 — 그래서 거절이 아니라 다시 질문해 달라는 안내로 문구가 갈린다.
NO_RELEVANT_EVIDENCE_MESSAGE = (
    "질문과 직접 관련된 예금보험공사 자료를 충분히 찾지 못해 정확한 안내가 어렵습니다. "
    "업무명이나 제도명을 포함해 다시 질문해 주세요."
)

SYSTEM_INSTRUCTION = f"""당신은 예금보험공사(KDIC)의 AI 상담 챗봇 "예솜24"입니다. 정확하고 신뢰할 수 있는 답변으로 국민을 돕는 것이 당신의 역할입니다.
...
9. 사용자가 "너는 누구야", "무슨 AI야", "HyperCLOVA X야?" 처럼 당신의 정체를 물으면, 모델명(HyperCLOVA X 등)이 아니라 "예금보험공사의 AI 상담 챗봇 예솜24"라고 답하세요."""
...
FEW_SHOT_EXAMPLES = [
    ...
    },
    {
        ...
        "answer": "안녕하세요! 저는 예금보험공사의 AI 상담 챗봇 예솜24입니다. 예금자보호제도나 착오송금 반환지원처럼 궁금하신 점을 편하게 물어봐 주세요.",
    },
    {
...
def build_civil_petition_prompt(query, civil_petition_answer):
    ...
    ⚠️ 답변 지시의 "적지 않아도 됩니다"를 **"하지 마세요"로 되돌리지 말 것.** 종전 문구
    ("서류·URL 언급은 하지 마세요")는 같은 문장 안에 예외("신청 링크를 달라는 질문이어도
    거절하지 말고")를 달고 있었는데, HCX-DASH-002 는 앞의 금지만 따르고 예외를 못 따랐다 —
    사용자가 금지 목록에 이름이 적힌 것(링크·URL)을 요구하면 "줄 게 없다"로 판단해 거절문을
    냈다. 2026-08-27 단일변수 실측(같은 근거·같은 모델, 질문만 교체):

        "신청 링크를 알려주세요"      3/3 거절      "신청 방법을 알려주세요"     0/3
        "신청 URL 알려주세요"         3/3 거절      "어디서 신청하나요?"        0/3
                                                  "신청 사이트 주소 알려주세요"  0/3

    시스템 원칙 5(URL 쓰지 말 것)는 무관하다 — 통째로 지워도 3/3 거절이고, 거기에 "링크를
    요구받아도 절차로 답하라"를 더해도 5/5 거절이었다. 이 꼬리 구절만 지우면 0/3 이 된다.
    금지를 면제로 바꾸면 거절 0/4 · URL 노출은 strip_urls 백스톱이 받는다.
    (HCX-007 은 종전 문구에서도 0/5 라 이 문제가 없다 — DASH-002 로 내리며 드러났다.)"""
    human = (
        f"{_format_examples()}\n\n"
        "--- 아래는 실제 질문입니다 ---\n\n"
        ...
        " 설명하세요 - 서류 목록과 링크는 시스템이 답변 뒤에 붙이므로 본문에 적지 않아도 됩니다. 신청 링크나 서류를 달라는 질문, '받을 수 있나요'처럼 가능 여부를 묻는 질문이어도 거절하지 말고 위 근거의 절차를 설명하세요 - 실제 링크와 서류 목록은 당신의 답변 뒤에 시스템이 따로 붙이므로 당신이 URL을 몰라도 사용자는 받게 됩니다. 근거가 질문과 다른 제도·기관 이야기라면"
...
# 앞의 공백까지 함께 먹는다 — 마크다운 링크([fins.kdic.or.kr](http://...))는 라벨과 주소가 둘 다
# 지워져 " []()" 가 되고, 괄호만 지우면 "홈페이지 에서" 처럼 조사가 떨어져 나온다(2026-08-27 실측).
