...
def record_gate1_span(canonical_text, rule_text, result):
    ...
    start_observation 이 현재 컨텍스트 아래에 이 span 을 끼운다 — Gate 1 이 파이프라인의 첫
    자식 span 이 된다. 웹 SSE 경로는 스레드풀 소비라 ambient 컨텍스트가 없어 이 방식이 안 되므로
    거긴 record_trace 메타데이터로 Gate 1 결과를 남긴다(그쪽은 이 함수를 부르지 않는다).
