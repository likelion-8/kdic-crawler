...
def build_document_section(chunks):
    ...
    다른 업무 페이지가 자주 섞이는데(testset intent=civil_petition **전수 263건** 실측
    47.1%), 종전에는 그 페이지의 서류까지 전부 걷어 왔다. 실물 두 가지 —
    ...
    전수 263건 중 41건(15.6%)의 서류 섹션이 달라진다. 부제 최대 길이 1307자 → 229자,
    질문당 카드 최대 3장(변화 없음). 남는 229자는 sender_docs 구비서류 12건 묶음으로,
    같은 업무라 정당하게 남는 것이다. 라벨 개수 상한은 두지 않았다 — 229자를 자를 임의
    임계값을 정할 근거가 없고, 그 12건은 실제로 준비해야 할 서류 목록이다.

    ⚠️ **트레이드오프: top-1 업무가 틀리면 정답 서류도 함께 떨어진다(실측 4/263).**
    떨어져 나간 카드 43장 중 39장은 타 업무(정답 제거 아님)지만, 4장은 테스트셋
    business_function 라벨과 같은 업무였다 — 검색이 top-1 을 다른 업무로 잡은 문항들이다
    (예: 라벨=착오송금 반환 신청인데 top-1 이 예금보험금 안내로 잡혀 "구비서류안내 -
    착오송금인"·"신청서 다운로드"가 사라졌다). 종전에는 정답 서류가 2~5위 페이지 덕에
    우연히 붙어 있던 경우다. 39:4 라 순효과는 개선이고 원인은 이 필터가 아니라 검색이
    top-1 을 틀린 것이지만, 검색 층을 손볼 때 이 값이 트레이드오프의 근거가 된다.

    (초기 측정은 앞 80건 부분집합이었다 — 58.8% / 28.7% / 1307→47자 / 카드 2→1장 이
    나왔고 "달라지는 방향은 전부 타 업무 제거"로 단정했으나, 전수에서는 넷 다 다르고
    마지막은 성립하지 않는다. 비교는 양쪽에 links 게이트를 똑같이 적용해야 의미가 있다 —
    빼고 build_document_section 만 비교하면 1307자 카드가 '후'에도 살아남은 것처럼 보인다.)"""
    ...
    for page_id in _unique_page_ids(chunks):
        ...
        # build_civil_petition_answer 가 서류 섹션을 통째로 생략한다. 현재 코퍼스
        # 58페이지는 전부 business_function 이 있어 이 분기는 실제로 타지 않는다(순수
        # 방어). 코퍼스에 결손이 생겨도 서류가 통째로 사라지지는 않게 두는 것이다.
