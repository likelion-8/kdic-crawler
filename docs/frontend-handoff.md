...
| D2 | 상태 칩의 실패는 서버가 **종류별로 나눠** `service.errors[{key,count}]`로 줄 것 — `ERROR_RATE`면 AD-005 실패 필터, `PIPELINE`이면 AD-004. FE는 key로만 분기한다. 0건인 종류는 싣지 않는다(비면 `level:'OK'`). **2026-09-01: 종전 `error_count`+`cause` 한 쌍을 대체했다** — 합계 한 숫자에 이동처가 하나뿐이라 두 종류가 겹친 날 한쪽이 가려졌고, 관리자 로그인 실패까지 합산돼 '서비스 오류 25건'이 3건짜리 목록을 열었다. 활동 로그 실패는 AD-011 소관이라 빠진다 | **FE정** | 요청 28 |
...
| D3' | 리소스 카드 표시 문자열($·M 등)은 서버가 완성해서 줄 것 — 통화·단위 표기를 프론트가 지어내지 않는다. **2026-08-26: 통화를 ₩ → $ 로 바꿨다**(`cost[].krw` → `cost[].usd`) — OpenAI 가 USD 원가이고, CLOVA 원화 공시가는 NCP 가 요금 페이지에 함께 적어 둔 당월 환율(1 KRW = 0.0006939 USD)로 옮긴다. 우리가 환율을 지어내지 않는다는 뜻이다. 요금표(`MODEL_PRICE_USD_PER_1M`)에 없는 모델은 `cost_breakdown[].share=null` + 토큰 수로 내려가고 목록 맨 뒤에 선다 — 0원으로 채우면 '공짜'로 읽힌다 | **FE정** | 요청 30 |
