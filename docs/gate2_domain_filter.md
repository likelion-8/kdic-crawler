...
- 웹(`api/rag/sse.py`): 가드레일 → **Gate 1**(원문) → 재작성(후속 턴) → 캐시 → **Gate 2**(재작성문)
  → 쿼리 플래너 순(2026-08-25 Gate 1 을 재작성·캐시 앞으로 이동). 웹도 2026-08-26 부터
  `gate2_embedding` span 을 남긴다 — 요청 시작에 연 `web_chat` 루트 span 밑에 `as_child_of`
  로 붙인다(threadpool 소비라 ambient trace 를 못 쓰는 사정은 그대로다).
