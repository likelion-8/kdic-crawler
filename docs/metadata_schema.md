...
> 모두 정의했다.
>
> **건수 기준: 2026-09-01** — 58페이지 · 502청크 · 849문항. 청크는 494(P2 종료) →
> 503(08-14 재청킹) → 502(페이징 UI 오염 제거)로 바뀌어 왔으므로, 옛 문서의 수치를
> 그대로 인용하지 말고 이 표를 기준으로 삼는다.
...
결과로 총 502개 청크 · 6개 필드로 구성된다. **corpus.jsonl에서 검색·라우팅에 필요한
...
```
안 쪼개진 페이지 (58개 중 39개):  {page_id}            예: dp_protlmts
쪼개진 페이지     (58개 중 19개):  {page_id}#{청크번호}  예: dp_fnst#0, dp_fnst#1
```
