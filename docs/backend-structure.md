...
| 26 | **복합 질문은 `sub_answers[]`로 내려라.** 마크다운 문자열(`**하위질문**\n답변`)을 `answer`에 이어붙이면 프론트가 파싱하지 않는다. 모양은 `sub_answers:[{title, answer, sources[], attachments[]}]`이고 **출처는 하위 블록마다 각각** 붙인다 — 하단에 모으면 어느 주장의 근거인지 끊긴다. **하위 간 출처 중복 제거 금지**(P1이 2026-07-30에 제거했다가 버그를 냈다). `sub_answers`가 있으면 최상위 `sources`는 빈 배열 (A-02 확정) | `src/pipeline.py:55-61, 130` |
