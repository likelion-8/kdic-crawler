...
- `src/inventory.py` — 수집 대상 정의, 팀 통합본 (담당분은 `owner == "dy"` 8건 + 업무분류·요약·필수여부·수집 근거). 여기 있는 것만 크롤한다
...
- 분석필요 2건(**ms_trgt_fnst 보험금 지급대상 금융회사**, **uc_hrpe_hist 상속인 금융거래조회**) 포함 확정 — 2026-07-13 팀 협의. 담당 8건 전체 수집
- ms_trgt_fnst는 검색·조회 기능 페이지라 동적 검색 결과는 스냅샷에 안 담김 — 안내 문구·기본 목록만 수집됨 (한계 인지)
