 ...
 *  ① '핵심 결과'의 지표 축은 대상별로 다르고 라벨까지 서버가 준다
 *  ② '대상'은 운영 설정 / RAG 초안 / 프롬프트 초안 3종이다
...
// ① 지표 축 — 프롬프트 계열은 회귀/인용/중대 위반, 그 외는 정확도/MRR/생성
for (const run of all.items) {
  if (run.status === 'RUNNING' || run.status === 'QUEUED') continue
  const labels = run.metrics.map((m) => m.label)
  assert.deepEqual(
    labels,
    run.target === '프롬프트 초안' ? ['회귀', '인용', '중대 위반'] : ['정확도', 'MRR', '생성'],
    `${run.run_id}의 지표 축이 대상과 맞지 않는다`,
  )
  assert.ok(run.metrics.every((m) => m.value !== ''), '표시값은 서버가 완성해 준다')
}

// ② 대상 어휘 — '운영'·'초안' 같은 축약형이 남아 있으면 안 된다
const targets = new Set(all.items.map((r) => r.target))
for (const t of targets) {
  assert.ok(['운영 설정', 'RAG 초안', '프롬프트 초안'].includes(t), `대상 값이 목업과 다르다: ${t}`)
}

// ③ 필터·페이지는 서버 몫. total은 자른 배열이 아니라 조건에 맞는 전체 건수다
const prompts = await get(`page=1&size=50&target=${encodeURIComponent('프롬프트 초안')}`)
assert.ok(prompts.total > 0 && prompts.total < all.total, '대상 필터가 서버에서 걸리지 않는다')
assert.ok(prompts.items.every((r) => r.target === '프롬프트 초안'))
