...
def test_url_backstop():
    ...
    # 마크다운 링크 형태 — 라벨과 주소가 둘 다 URL 이면 양쪽이 지워지고 빈 괄호만 남아,
    # 앞의 공백 + 조사가 "홈페이지 에서" 로 떠 있었다(2026-08-27 DASH-002 실측). 민원 프롬프트의
    # URL 금지문을 면제로 바꾸면서 본문에 주소가 들어오는 빈도가 늘어 이 형태가 실제로 나온다.
    assert strip_urls("공식 홈페이지 [fins.kdic.or.kr](http://fins.kdic.or.kr)에서 신청합니다.") \
        == "공식 홈페이지에서 신청합니다.", "빈 괄호 앞 공백이 남아 조사가 떨어져 나옴"
