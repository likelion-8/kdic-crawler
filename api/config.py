...
class Settings(BaseSettings):
    ...
    # --- 타임아웃(초) -----------------------------------------------------
    # RAG 한 번은 LLM 을 여러 번 부른다(분해 판단 1회 + 하위질문마다 답변 생성 1회
    # + [NO_SOURCE] 인 경우 출처 재확인 1회). 복합 질문이면 30초를 넘길 수 있어
    # 넉넉히 잡는다. src/pipeline.py 의 USE_QUERY_DECOMPOSITION / USE_SOURCE_RECHECK 를
    # 끄면 이 값을 줄여도 된다.
    rag_timeout_s: float = 120.0
    # SSE 스트리밍 중 연결 유지를 위한 주석(comment) 이벤트 간격. 프록시/로드밸런서가
    # 유휴 연결을 끊지 않도록 하는 용도로 api/rag/sse.py 가 쓴다.
